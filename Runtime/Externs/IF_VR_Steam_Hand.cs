...
namespace InterVR.IF.VR.Plugin.Steam.Extern
{
    //-------------------------------------------------------------------------
    ...
    public class IF_VR_Steam_Hand : MonoBehaviour
    {
        ...
        public struct AttachedObject
        {
            ...
            public IF_VR_GrabType grabbedWithType;
            ...
            public Valve.VR.InteractionSystem.AllowTeleportWhileAttachedToHand allowTeleportWhileAttachedToHand;
        ...
        public Valve.VR.InteractionSystem.AllowTeleportWhileAttachedToHand currentAttachedTeleportManager
        {
            get
            {
        ...
        //-------------------------------------------------
        public void AttachObject(GameObject objectToAttach, IF_VR_GrabType grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            attachedObject.allowTeleportWhileAttachedToHand = objectToAttach.GetComponent<Valve.VR.InteractionSystem.AllowTeleportWhileAttachedToHand>();
        ...
        protected virtual void FixedUpdate()
        {
            if (currentAttachedObject != null)
            {
                ...
                if (attachedInfo.attachedObject != null)
                {
                    ...
                    {
                        float t = Valve.VR.InteractionSystem.Util.RemapNumberClamped(Time.time, attachedInfo.attachTime, attachedInfo.attachTime + attachedInfo.interactable.snapAttachEaseInTime, 0.0f, 1.0f);
                        if (t < 1.0f)
                        {
                            if (attachedInfo.HasAttachFlag(AttachmentFlags.VelocityMovement))
                            {
        ...
        protected virtual void OnInputFocus(bool hasFocus)
        {
            ...
            else
            {
                ...
                AttachObject(applicationLostFocusObject, IF_VR_GrabType.Scripted, AttachmentFlags.ParentToHand);
        ...
        {
            //Valve.VR.InteractionSystem.ControllerButtonHints.ShowButtonHint(this, grabGripAction); //todo: assess
        }

        public void HideGrabHint()
        {
            //Valve.VR.InteractionSystem.ControllerButtonHints.HideButtonHint(this, grabGripAction); //todo: assess
        }

        public void ShowGrabHint(string text)
        {
            //Valve.VR.InteractionSystem.ControllerButtonHints.ShowTextHint(this, grabGripAction, text);
        }

        public IF_VR_GrabType GetGrabStarting(IF_VR_GrabType explicitType = IF_VR_GrabType.None)
        {
            if (explicitType != IF_VR_GrabType.None)
            {
                if (noSteamVRFallbackCamera)
                {
                ...
                if (explicitType == IF_VR_GrabType.Pinch && grabPinchAction.GetStateDown(handType))
                    return IF_VR_GrabType.Pinch;
                if (explicitType == IF_VR_GrabType.Grip && grabGripAction.GetStateDown(handType))
        ...
        public IF_VR_GrabType GetGrabEnding(IF_VR_GrabType explicitType = IF_VR_GrabType.None)
        {
            if (explicitType != IF_VR_GrabType.None)
            {
                if (noSteamVRFallbackCamera)
                ...
                if (explicitType == IF_VR_GrabType.Pinch && grabPinchAction.GetStateUp(handType))
                    return IF_VR_GrabType.Pinch;
                if (explicitType == IF_VR_GrabType.Grip && grabGripAction.GetStateUp(handType))
                    return IF_VR_GrabType.Grip;
            }
            else
            {
        ...
        public bool IsGrabbingWithType(IF_VR_GrabType type)
        {
            if (noSteamVRFallbackCamera)
            {
            ...
            switch (type)
            {
                case IF_VR_GrabType.Pinch:
                    return grabPinchAction.GetState(handType);

                case IF_VR_GrabType.Grip:
        ...
        public bool IsGrabbingWithOppositeType(IF_VR_GrabType type)
        {
            if (noSteamVRFallbackCamera)
            {
                if (Input.GetMouseButton(0))
        ...
        public IF_VR_GrabType GetBestGrabbingType()
        {
            return GetBestGrabbingType(IF_VR_GrabType.None);
        }

        public IF_VR_GrabType GetBestGrabbingType(IF_VR_GrabType preferred, bool forcePreference = false)
        {
            if (noSteamVRFallbackCamera)
            {
                if (Input.GetMouseButton(0))
            ...
            if (preferred == IF_VR_GrabType.Pinch)
            {
            ...
            if (preferred == IF_VR_GrabType.Grip)
            {
    ...
    [System.Serializable]
    public class HandEvent : UnityEvent<IF_VR_Steam_Hand> { }
}