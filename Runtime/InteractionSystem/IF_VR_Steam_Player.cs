...
//
// Purpose: IF_VR_Steam_Player interface used to query HMD transforms and VR hands
//
//=============================================================================
...
namespace InterVR.IF.VR.Plugin.Steam.InteractionSystem
{
    ...
    public class IF_VR_Steam_Player : MonoBehaviour
	{
		...
		//-------------------------------------------------
		// Singleton instance of the IF_VR_Steam_Player. Only one can exist at a time.
		//-------------------------------------------------
		private static IF_VR_Steam_Player _instance;
		public static IF_VR_Steam_Player instance
		{
			get
			{
				if ( _instance == null )
				{
					_instance = FindObjectOfType<IF_VR_Steam_Player>();
				}
				return _instance;
			}
		}
		...
		//-------------------------------------------------
		// Get the i-th active IF_VR_Steam_Hand.
		//
		// i - Zero-based index of the active IF_VR_Steam_Hand to get
		//-------------------------------------------------
		public IF_VR_Steam_Hand GetHand( int i )
		{
		...
		//-------------------------------------------------
		public IF_VR_Steam_Hand leftHand
		{
			get
			{
				for ( int j = 0; j < hands.Length; j++ )
		...
		//-------------------------------------------------
		public IF_VR_Steam_Hand rightHand
		{
			get
			{
				for ( int j = 0; j < hands.Length; j++ )
        ...
        //-------------------------------------------------
        // Get IF_VR_Steam_Player scale. Assumes it is scaled equally on all axes.
        //-------------------------------------------------
		...
		void OnDrawGizmos()
		{
			...
			{
				IF_VR_Steam_Hand hand = GetHand( i );
				...
				{
                    /*
					IF_VR_Steam_Hand.HandType guessHandType = hand.currentHandType;

					if ( guessHandType == IF_VR_Steam_Hand.HandType.Left )
					{
						Gizmos.DrawIcon( hand.transform.position, "vr_interaction_system_left_hand_question.png" );
					}
					else if ( guessHandType == IF_VR_Steam_Hand.HandType.Right )
					{
